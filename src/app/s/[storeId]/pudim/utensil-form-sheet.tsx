...
import type { PudimUtensil, StockItem } from "@/lib/types";
import { createPudimUtensilAction, updatePudimUtensilAction } from "@/actions/pudim";
...
function UtensilForm({
  ...
  function submit() {
    ...
    startTransition(async () => {
      const result = utensil
        ? await updatePudimUtensilAction(utensil.id, { ...data, archived: utensil.archived })
        : await createPudimUtensilAction(data);
      if (result.ok) {
  ...
  function toggleArchived() {
    if (!utensil) return;
    startTransition(async () => {
      const result = await updatePudimUtensilAction(utensil.id, {
        storeId,
        name: utensil.name,
        insumo: {
  ...
  return (
    <>
      <div className="flex-1 space-y-5 p-4">
        ...
        <div className="space-y-1.5">
          <Label htmlFor="utensil-qty">Quantidade por pudim</Label>
          <Input
            id="utensil-qty"
            value={qty}
        ...
        <div className="flex items-center justify-between rounded-xl border border-border bg-card px-3.5 py-3">
          <div>
            <p className="text-[13px] font-semibold text-ink">Padrão em todo pudim</p>
