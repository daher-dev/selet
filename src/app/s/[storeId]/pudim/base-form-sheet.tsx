...
import type { PudimBase, StockItem } from "@/lib/types";
import { createPudimBaseAction, updatePudimBaseAction } from "@/actions/pudim";
...
function BaseForm({
  ...
  function submit() {
    ...
    startTransition(async () => {
      const result = base
        ? await updatePudimBaseAction(base.id, { ...data, archived: base.archived })
        : await createPudimBaseAction(data);
      if (result.ok) {
  ...
  function toggleArchived() {
    if (!base) return;
    startTransition(async () => {
      const result = await updatePudimBaseAction(base.id, {
        storeId,
        name: base.name,
    ...
    <>
      <div className="flex-1 space-y-5 p-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            <Label htmlFor="base-qty">Quantidade por pudim</Label>
            <Input
              id="base-qty"
              value={qty}
