...
import type { Product, PudimBrinde } from "@/lib/types";
import { cn } from "@/lib/utils";
import { formatBRL } from "@/lib/format";
import { PRODUCT_CATEGORY_META } from "@/components/category-meta";
import { EmptyState } from "@/components/ui/empty-state";
import { setPudimBrindeArchivedAction } from "@/actions/pudim";
import { StatusPill } from "../shakes/shakes-client";
 ...
 * used by bases/adicionais/utensílios — this is a bespoke grid.
 */
  ...
  if (brindes.length === 0) {
    return (
      <EmptyState
        ...
        description="Adicione itens do cardápio para saírem de graça junto com o pudim (ex.: Chá Limão)."
        action={
          <button
            type="button"
            onClick={onCreate}
...
function BrindeCard({
  ...
  function toggleArchived() {
    startTransition(async () => {
      const result = await setPudimBrindeArchivedAction({
        storeId,
  ...
  return (
    ...
    >
      ...
      <div className="mt-auto flex items-center gap-2 rounded-[10px] border border-mint-wash bg-mist px-3 py-2.5">
        <span className="flex-1 text-[12px] text-ink-soft">Com o pudim</span>
