...
import type { PriceTier, ShakeMixin, ShakeRim, StockItem } from "@/lib/types";
import { formatBRL } from "@/lib/format";
import {
  createShakeMixinAction,
  createShakeRimAction,
  updateShakeMixinAction,
  updateShakeRimAction,
...
type Kind = "borda" | "adicional";

const KIND_META: Record<
  Kind,
  { eyebrow: string; newTitle: string; namePlaceholder: string }
> = {
  borda: { eyebrow: "Borda do copo", newTitle: "Nova borda", namePlaceholder: "Nutella" },
  adicional: {
    eyebrow: "Adicional do shake",
    newTitle: "Novo adicional",
    namePlaceholder: "Fibra Ativa",
  },
};

interface TieredModifierFormSheetProps {
  kind: Kind;
  storeId: string;
  modifier: ShakeRim | ShakeMixin | null;
  stockItems: StockItem[];
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function TieredModifierFormSheet({
  kind,
  storeId,
  modifier,
  stockItems,
      ...
      <SheetContent side="right" className="w-full gap-0 overflow-y-auto sm:max-w-md">
        <SheetHeader className="border-b border-border">
          ...
          <SheetTitle className="text-[17px] font-bold">
            {modifier ? modifier.name : meta.newTitle}
          </SheetTitle>
        </SheetHeader>
        {open && (
          <TieredModifierForm
...
function createAction(
  ...
  return kind === "borda" ? createShakeRimAction(input) : createShakeMixinAction(input);
}
...
function updateAction(
  ...
  return kind === "borda"
    ? updateShakeRimAction(id, input)
    : updateShakeMixinAction(id, input);
}

function TieredModifierForm({
  kind,
  storeId,
  modifier,
  stockItems,
  ...
  function submit() {
    ...
    startTransition(async () => {
      const result = modifier
        ? await updateAction(kind, modifier.id, { ...data, archived: modifier.archived })
        : await createAction(kind, data);
      if (result.ok) {
  ...
  function toggleArchived() {
    ...
    startTransition(async () => {
      ...
      const result = await updateAction(kind, modifier.id, data);
      if (result.ok) {
  ...
  return (
    <>
      <div className="flex-1 space-y-5 p-4">
        ...
        <div className="space-y-1.5">
          <Label htmlFor="mod-qty">Quantidade por shake</Label>
          <Input
            id="mod-qty"
            value={qty}
